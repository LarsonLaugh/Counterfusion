...
def theta(Om1, Om2, nfm):
    m = Om1.shape[0]
    th = np.zeros((m, m))
    pm = pmatrix(Om1, Om2, nfm)
    qm = qmatrix(Om1, Om2, nfm)
    lm = lmatrix(Om2, nfm)
    detp = det(pm)
    for k in range(m):
        for i in range(m):
            if k < nfm and i < nfm:
                th[k, i] = Om1[k, i] + sum(
                    [
                        Om1[k, j] * det(replace_colnum(pm, j - nfm, qm[:, i])) / detp
                ...
                th[k, i] = sum(
                    [
                        Om1[k, j]
                        * det(replace_colnum(pm, j - nfm, lm[:, i - nfm]))
                        / detp
                        for j in range(nfm, m)
                    ]
                )
            elif k >= nfm and i < nfm:
                th[k, i] = det(replace_colnum(pm, k - nfm, qm[:, i])) / detp
            else:
                th[k, i] = det(replace_colnum(pm, k - nfm, lm[:, i - nfm])) / detp
    return th
    ...
    for k in range(m):
        for i in range(m):
            if k < nfm and i < nfm:
                ...
                Om3[k, i] = (
                    sum([Om1[k, j] * th[j, i] for j in range(nfm, m)]) + Om1[k, i]
                )
            else:
...
def interaction_builder(dimension, id1, id2, value):
    if isinstance(value, (int, float)):
        ...
        raise ValueError(
            "value needs to be either a float/int (doubly stochastic matrix) or a list including 2 "
